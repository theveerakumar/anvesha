...
from sqlalchemy import BigInteger, Date, DateTime, Float, ForeignKey, String, Text, func
...
class Fund(Base):
    ...
    nav_history: Mapped[list["FundNAVHistory"]] = relationship(back_populates="fund", cascade="all, delete-orphan")
...
class FundNAVHistory(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    fund_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("funds.id", ondelete="CASCADE"), index=True)
